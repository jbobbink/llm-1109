...
interface SentimentScoresData {
    brandName: string;
    sentiments: Partial<Record<Provider, { P: number, N: number, Nl: number }>>;
}
      ...
      <div className="overflow-x-auto pr-2">
        <table className="w-full text-left">
          <thead className="sticky top-0 bg-gray-800 z-10">
            <tr>
              <th rowSpan={2} className="text-sm font-semibold text-gray-400 pb-2 border-b-2 border-gray-700 align-bottom">Brand</th>
              {config.providers.map(p => (
                  <th key={p} colSpan={3} className="text-sm font-semibold text-gray-400 pb-2 border-b-2 border-gray-700 text-center" title={config.models[p]}>{getProviderShortName(p, config)}</th>
              ))}
            </tr>
            <tr>
              {config.providers.map(p => (
                <React.Fragment key={p}>
                  <th className="text-xs font-medium text-green-400 pb-2 border-b border-gray-700 text-center" title="Positive">P</th>
                  <th className="text-xs font-medium text-gray-400 pb-2 border-b border-gray-700 text-center" title="Neutral">N</th>
                  <th className="text-xs font-medium text-red-400 pb-2 border-b border-gray-700 text-center" title="Negative">N</th>
                </React.Fragment>
              ))}
            </tr>
          </thead>
                    ...
                    {config.providers.map(p => {
                        ...
                        return (
                            <React.Fragment key={p}>
                                ...
                                <td className="py-3 text-center text-gray-400 font-mono">{score.Nl}</td>
