    ...
    return (
        <div className="max-w-2xl mx-auto bg-gray-800 p-8 rounded-xl shadow-lg border border-gray-700">
            ...
            <div className="space-y-3 max-h-80 overflow-y-auto pr-2">
                {tasks.map(task => (
                    <div key={task.id} className={`p-3 rounded-lg flex items-center space-x-4 transition-colors ${task.status === 'completed' ? 'bg-green-900/30' : 'bg-gray-700'}`}>
                        ...
                        <div className="flex-grow">
                            <p className={`text-sm font-medium ${task.status === 'error' ? 'text-red-400' : 'text-gray-300'}`}>
                                {task.description}
                                {task.retries && task.status === 'in_progress' && (
                                    <span className="ml-2 text-yellow-400 text-xs font-semibold">(Retrying {task.retries}/2...)</span>
                                )}
                            </p>
